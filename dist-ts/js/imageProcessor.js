...
class ImageProcessor {
    ...
    updateSettings(newSettings) {
        ...
        const bUpdatePalette = (newSettings.grayscale != this.settings.grayscale
    ...
    getAverageColor(pixelX, pixelY, size) {
        ...
        if (count === 0)
            return color(0, 0, 0);
        return color(Math.floor(r / count), Math.floor(g / count), Math.floor(b / count));
    }
    ...
    updateRenderGraphics(gfx, renderPixelSize = 8) {
        if (!this.processedPixels.length)
            return;
        gfx.noStroke();
        gfx.background(0);
        // == Pixels processing
        for (let pixel of this.processedPixels) {
            ...
            gfx.rect(pixel.x * renderPixelSize, pixel.y * renderPixelSize, renderPixelSize, renderPixelSize);
            // == Scanlines
    ...
    applyScanlines(gfx, pixel, renderPixelSize) {
        ...
        gfx.rect(pixel.x * renderPixelSize, pixel.y * renderPixelSize + renderPixelSize / 2, renderPixelSize, renderPixelSize / 2);
        gfx.blendMode(BLEND);
    }
    ...
    exportImage() {
        ...
        const exportGfx = createGraphics(dimensions.width * finalPixelSize, dimensions.height * finalPixelSize);
        exportGfx.noStroke();
        exportGfx.background(0);
        for (let pixel of this.processedPixels) {
            exportGfx.fill(pixel.color);
            exportGfx.rect(pixel.x * finalPixelSize, pixel.y * finalPixelSize, finalPixelSize, finalPixelSize);
            if (this.settings.scanlines)
