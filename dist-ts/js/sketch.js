...
function getProcessingMode() {
    ...
    if (settings.colorLimit)
        return `Limited Colors (${settings.colors})`;
    if (settings.customPalette)
        return `${settings.paletteType} Palette`;
    return 'Full Color';
}
function getActiveEffects() {
    ...
    if (settings.dithering)
        effects.push(`${settings.ditheringType} Dithering`);
    if (settings.scanlines)
        effects.push('Scanlines');
    if (settings.overlayMode > 0)
        effects.push('Overlay');
