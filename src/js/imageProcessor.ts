import { SCANLINES_DATA } from './globals';
...
// Déclarations pour les fonctions utilitaires
declare function toGrayscale(r: number, g: number, b: number): number;
declare function getBayerValue(matrixName: string, x: number, y: number): number;
declare function applyGrayscaleDithering(grayValue: number, matrixName: string, x: number, y: number): number;
declare function findNearestColor(color: any, palette: any[]): any;
declare function kMeansQuantize(colors: any[], k: number): any[];
declare function getPalette(type: string): any[];
